...
class ATM {
    ...
    private void performDeposit(JTextArea outputArea) {
        ...
        if (accountNumber == null) return; // Handle cancellation

        String amountString = JOptionPane.showInputDialog("Enter amount to deposit:");
        if (amountString == null) return; // Handle cancellation
    ...
    private void performWithdrawal(JTextArea outputArea) {
        // Prompt user for account number and withdrawal amount
        String accountNumber = JOptionPane.showInputDialog("Enter account number:");
        ...
        try {
            ...
            boolean found = false; // Flag to check if the account was found

            // Loop through the user's accounts to find the specified account
            for (Account account : currentUser.getAccounts()) {
                if (account.getAccountNumber().equals(accountNumber)) {
                    // Attempt to withdraw the specified amount
                    account.withdraw(amount); // This may throw InsufficientFundsException
                    transactions.add(new Transaction(UUID.randomUUID().toString(), accountNumber, amount, "Withdrawal"));
                    updateTransactionFile(); // Update the transaction log
                    ...
                    found = true; // Account found and withdrawal processed
...
private void performTransfer(JTextArea outputArea) {
    // Prompt for account numbers and transfer amount
    ...
    try {
        ...
        // Optionally, update the balances in the output
...
private void loadTransactionsFromFile() {
    try (Scanner fileScanner = new Scanner(new File(transactionFile))) {
        while (fileScanner.hasNextLine()) {
            ...
            String[] data = line.split(",");
            if (data.length == 4) { // Ensure there are exactly 4 elements
                try {
